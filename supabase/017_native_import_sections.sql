...
begin
  ...
  if v_category_mode = 'replace_all' then
    update public.categories c set active = false, updated_at = now(), updated_by = auth.uid()
      where c.scope = 'site' and c.owner_id is null and not exists (
        ...
        where x->>'type' = c.type and public.normalize_catalog_name(x->>'name') = public.normalize_catalog_name(c.name));
    get diagnostics v_category_removed = row_count;
  end if;
  for v_item in select * from jsonb_array_elements(coalesce(p_categories, '[]'::jsonb)) loop
    select id into v_id from public.categories where scope = 'site' and owner_id is null and type = v_item->>'type' and public.normalize_catalog_name(name) = public.normalize_catalog_name(v_item->>'name') limit 1;
  ...
  for v_item in select * from jsonb_array_elements(coalesce(p_products, '[]'::jsonb)) loop
    select id into v_category_id from public.categories where scope = 'site' and owner_id is null and active and type = 'proteina' and public.normalize_catalog_name(name) = public.normalize_catalog_name(v_item->>'category') limit 1;
