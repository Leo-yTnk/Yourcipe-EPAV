...
-- automatically; run this manually in the Supabase SQL Editor. This script
-- is idempotent — safe to run it again after a partial run or a later change.
--
...
-- supabase/optional_products_recipes_future.sql. They don't exist yet
-- (the app still stores that data in localStorage), so this file never
-- references them.
...
--   'anon'          — not logged in (can't reach this trigger anyway,
--                      since anon has no UPDATE grant on this table at all) -> BLOCKED
...
--
-- IMPORTANT: do NOT write this as `coalesce(auth.role(), '') <> 'service_role'`.
-- That looks equivalent but is not: coalesce() turns NULL into '', and
-- '' <> 'service_role' is TRUE, so it would ALSO block the SQL Editor
-- (auth.role() IS NULL there) — silently reverting the admin's own manual
-- promotion. This was caught by actually running the promotion as postgres
-- against a local Postgres instance (see PR description) — the UPDATE
-- reported "UPDATE 1" (no permission error) but the role silently stayed
-- 'user'. The form below keeps NULL propagating through AND as intended:
-- `auth.role() is not null and auth.role() <> 'service_role'` is only true
-- for a real, non-service JWT role — NULL (SQL Editor) short-circuits to
-- false and is allowed through, exactly like `service_role` is.
