-- Yourcipe — category replacement takes precedence over uniqueness collisions.
-- Idempotent follow-up for environments where migration 018 was already applied.
-- Equivalent category/section rows are serialized and resolved to the existing
-- (type, slug) identity so "upsert" can never lose a race to uniqueness checks.
...
begin
  ...
  -- Category replacement and insertion must share one transaction-level lock.
  -- Otherwise two imports can both validate absence before either inserts and
  -- the losing request is blocked by categories_site_slug_uk instead of upserting.
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtext('admin_import_public_catalog:categories'));
  ...
  for v_item in select * from jsonb_array_elements(coalesce(p_categories, '[]'::jsonb)) loop
    select id into v_id from public.categories where scope = 'site' and owner_id is null and type = v_item->>'type' and (slug = public.slugify(v_item->>'name') or public.normalize_catalog_name(name) = public.normalize_catalog_name(v_item->>'name'))
      ...
      limit 1;
    if v_id is not null and v_category_mode = 'add' then
      v_category_ignored := v_category_ignored + 1;
    elsif v_id is not null then
      update public.categories set name = btrim(v_item->>'name'), active = true, updated_at = now(), updated_by = auth.uid() where id = v_id;
      v_category_replaced := v_category_replaced + 1;
    elsif v_category_mode = 'add' then
      insert into public.categories(scope, owner_id, type, name, slug, active, created_by, updated_by)
      values ('site', null, v_item->>'type', btrim(v_item->>'name'), public.slugify(v_item->>'name'), true, auth.uid(), auth.uid())
      on conflict (type, slug) where scope = 'site' do nothing;
      if found then v_category_added := v_category_added + 1; else v_category_ignored := v_category_ignored + 1; end if;
    else
      -- The unique index is the final authority for equivalence. If a row was
      -- created after lookup (or stale catalog data hid it), replacement wins:
      -- reactivate/update that row rather than surfacing a duplicate error.
      insert into public.categories(scope, owner_id, type, name, slug, active, created_by, updated_by)
      values ('site', null, v_item->>'type', btrim(v_item->>'name'), public.slugify(v_item->>'name'), true, auth.uid(), auth.uid())
      on conflict (type, slug) where scope = 'site' do update set
        name = excluded.name, active = true, updated_at = now(), updated_by = auth.uid()
      returning (xmax = 0) into v_was_added;
      if v_was_added then v_category_added := v_category_added + 1; else v_category_replaced := v_category_replaced + 1; end if;
    end if;
  end loop;
