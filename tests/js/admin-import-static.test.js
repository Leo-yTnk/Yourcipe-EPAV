...
describe('admin spreadsheet import wiring', () => {
  ...
  it('renders Home sections from the Supabase-backed ordered section list', () => {
    expect(app).toContain('homeSectionBlocks');
    expect(template).toContain('v.homeSectionBlocks.map');
    expect(template).toContain('Mover seção para cima');
    expect(app).toContain('catalog.adminReorderHomeSections');
  });
