...
describe('admin spreadsheet import wiring', () => {
  ...
  it('resolves imported recipe tags only to Home or Recipes section types', () => {
    const pageSpecificImportSql = fs.readFileSync('supabase/026_page_specific_import_sections.sql', 'utf8');
    expect(app).toContain("'secao_home', 'secao_receita', 'secao_produto'");
    expect(app).toContain("{ tipo: 'secao_receita', nome: 'Receitas Veganas' }");
    expect(pageSpecificImportSql).toContain("type in ('secao_receita', 'secao_home')");
