...
describe('admin spreadsheet import wiring', () => {
  ...
  const nativeSlugFixSql = fs.readFileSync('supabase/018_fix_native_section_slug_conflict.sql', 'utf8');
  ...
  it('accepts every native recipe tag without requiring spreadsheet categories', () => {
    ...
    expect(app).toContain('NATIVE_RECIPE_SECTION_NAMES.get(x.key) || x.raw');
    ...
    expect(nativeSectionsSql).toContain('slug = public.slugify(v_section.name)');
    expect(nativeSlugFixSql).toContain('slug = public.slugify(v_section.name)');
  });
