...
const NATIVE_RECIPE_SECTION_NAMES = new Map(SECTION_DEFS.map(section => [section.key, section.label]));
...
class App extends Component {
  ...
  processImportWorkbook = (wb, fileName) => {
    ...
    recipeRows.forEach((row, i) => {
      ...
      const sections = normalizedTags.filter(x => x.key !== 'destaque').filter(x => { const ok = NATIVE_RECIPE_TAGS.has(x.key) || sectionKeys.has(x.key); if (!ok) warnings.push(`Receitas, linha ${line}: seção "${x.raw}" não cadastrada; será ignorada.`); return ok; }).map(x => NATIVE_RECIPE_SECTION_NAMES.get(x.key) || x.raw);
