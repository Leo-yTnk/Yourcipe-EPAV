  ...
  processImportWorkbook = (wb, fileName) => {
    ...
    categoryRows.forEach((row, i) => {
      ...
      if (!name) errors.push(`Categorias, linha ${i + 2}: campo nome ausente.`);
        ...
        const typeLabel = type === 'proteina' ? 'categoria de produto' : type === 'receita' ? 'categoria de receita' : type === 'secao_receita' ? 'seção de receitas' : type === 'secao_produto' ? 'seção de produtos' : 'seção da home';
    ...
    const sectionKeys = new Set([
      ...
      ...[...categoryKeys].filter(k => k.startsWith('secao_receita:') || k.startsWith('secao_home:')).map(k => k.slice(k.indexOf(':') + 1)),
    ]);
