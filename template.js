import { html } from './vendor/htm-preact-standalone.js?v=20260831-2';
import { CustomSelect } from './custom-select.js?v=20260831-2';
  ...
  return html`
    <div style="padding:40px 40px 24px">
      ...
      ${v.hasSession && html`
        ...
        `}
      `}
      <div style="text-align:center;color:var(--neutral-400);font-size:12px;margin-top:32px;padding-bottom:4px">V0.41</div>
    </div>
  `;
}
...
function renderSiteProductsTab(app, v) {
  return html`
    <div style="padding:8px 40px 24px">
      ...
      ${v.swiftSyncReport && html`<section aria-live="polite" style=${`margin-bottom:16px;padding:14px 16px;border-radius:var(--radius-md);border:1px solid ${v.swiftSyncReport.kind === 'error' ? 'var(--red-500)' : 'var(--green-500)'};background:${v.swiftSyncReport.kind === 'error' ? 'rgba(195,61,34,.08)' : 'rgba(52,178,62,.08)'}`}>
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><strong>${v.swiftSyncReport.title}</strong><div style="font-size:13px;margin-top:3px;color:var(--neutral-700)">${v.swiftSyncReport.summary}</div></div><button type="button" onClick=${v.onToggleSwiftSyncDetails} aria-expanded=${v.swiftSyncDetailsOpen} style="border:0;background:transparent;color:var(--brand-700);font-weight:700;cursor:pointer;padding:2px">${v.swiftSyncDetailsOpen ? 'Ocultar detalhes' : 'Ver detalhes'}</button></div>
        ${v.swiftSyncDetailsOpen && html`<div style="margin-top:14px;border-top:1px solid var(--neutral-200);padding-top:10px"><ol style="list-style:none;padding:0;margin:0;display:grid;gap:9px">${v.swiftSyncReport.steps.map(step => html`<li style="display:grid;grid-template-columns:20px 1fr;gap:8px;font-size:13px"><span aria-hidden="true" style=${`font-weight:800;color:${step.status === 'success' ? 'var(--green-600)' : step.status === 'skipped' ? 'var(--neutral-500)' : 'var(--red-600)'}`}>${step.status === 'success' ? '✓' : step.status === 'skipped' ? '–' : '✕'}</span><span><strong>${step.label}</strong><br/><span style="color:var(--neutral-600)">${step.detail}</span></span></li>`)}</ol>${v.swiftSyncReport.failures.length > 0 && html`<div style="margin-top:12px"><strong style="font-size:13px">Falhas por produto</strong><ul style="margin:6px 0 0;padding-left:20px;font-size:12px;color:var(--red-600)">${v.swiftSyncReport.failures.map(item => html`<li><strong>${item.product}:</strong> ${item.code}</li>`)}</ul></div>`}</div>`}
      </section>`}
